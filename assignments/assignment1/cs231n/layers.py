...
def affine_backward(dout, cache):
    ...
    dw = x.reshape(x.shape[0], -1).T.dot(dout)#.T.dot(dout)
...
def relu_backward(dout, cache):
    ...
    # selected = np.where(x>0, 1, 0)
    # dx = np.multiply(selected, dout)
...
def svm_loss(x, y):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N = x.shape[0]
    margin = x - x[np.arange(N), y][:, np.newaxis] + 1
    margin[np.arange(N), y] *= 0
    loss = np.sum(np.maximum(margin, 0))
    loss /= N

    dx = np.where(margin > 0, 1.0, 0.0)
...
def softmax_loss(x, y):
    ...
    dx = softmax - mask # formula for the derivative of the loss wrt scores (x)
    dx /= N # gradients are averaged as well!! 
